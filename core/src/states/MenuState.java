...
public class MenuState implements Screen {
	...
	public boolean isOnScoreBtn() {
		return 10 <= Gdx.input.getX() && 10 + scoreBtn.getWidth() / 2 >= Gdx.input.getX()
	...
	public void render(float delta) {
		...
		game.getBatch().draw(exitBtn, GameProgMeth.WIDTH - 10 - exitBtn.getWidth() / 2, 10, exitBtn.getWidth() / 2,
				exitBtn.getHeight() / 2);
		game.getBatch().draw(playBtn, GameProgMeth.WIDTH / 2 - playBtn.getWidth() / 2,
				GameProgMeth.HEIGHT / 2 - (1.5f) * playBtn.getHeight());
		game.getBatch().end();
