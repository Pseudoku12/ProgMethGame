...
public class HallOfFameScene implements Screen {
	...
	public void render(float delta) {
		...
		game.getBatch().begin();
		game.getBatch().draw(nextBtn, GameProgMeth.WIDTH - 10 - nextBtn.getWidth(), 10, nextBtn.getWidth(),
				nextBtn.getHeight());
		...
		fontList.draw(game.getBatch(), Integer.toString(scores[0]), GameProgMeth.WIDTH / 2 + 100,
				GameProgMeth.HEIGHT - 300);
		fontList.draw(game.getBatch(), names[1], GameProgMeth.WIDTH / 2 - 200, GameProgMeth.HEIGHT - 350);
		fontList.draw(game.getBatch(), Integer.toString(scores[1]), GameProgMeth.WIDTH / 2 + 100,
				GameProgMeth.HEIGHT - 350);
		fontList.draw(game.getBatch(), names[2], GameProgMeth.WIDTH / 2 - 200, GameProgMeth.HEIGHT - 400);
		fontList.draw(game.getBatch(), Integer.toString(scores[2]), GameProgMeth.WIDTH / 2 + 100,
				GameProgMeth.HEIGHT - 400);
		fontList.draw(game.getBatch(), names[3], GameProgMeth.WIDTH / 2 - 200, GameProgMeth.HEIGHT - 450);
		fontList.draw(game.getBatch(), Integer.toString(scores[3]), GameProgMeth.WIDTH / 2 + 100,
				GameProgMeth.HEIGHT - 450);
		fontList.draw(game.getBatch(), names[4], GameProgMeth.WIDTH / 2 - 200, GameProgMeth.HEIGHT - 500);
		fontList.draw(game.getBatch(), Integer.toString(scores[4]), GameProgMeth.WIDTH / 2 + 100,
				GameProgMeth.HEIGHT - 500);
		game.getBatch().end();
	}
